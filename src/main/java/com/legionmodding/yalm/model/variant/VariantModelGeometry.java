package com.legionmodding.yalm.model.variant;
...
public class VariantModelGeometry implements IModelGeometry<VariantModelGeometry>
{
    ...
    {
        final IBakedModel base = this.base.bake(bakery, spriteGetter, modelTransform, modelLocation);
        List<Pair<Predicate<VariantModelState>, IBakedModel>> parts = this.parts.stream().map(p -> Pair.of(p.getFirst(), p.getSecond().bake(bakery, spriteGetter, modelTransform, modelLocation))).collect(Collectors.toList());
    ...
    public Collection<RenderMaterial> getTextures(IModelConfiguration owner, Function<ResourceLocation, IUnbakedModel> modelGetter, Set<Pair<String, String>> missingTextureErrors)
    {
        return Stream.concat(
                base.getMaterials(modelGetter, missingTextureErrors).stream(),
                parts.stream().flatMap(p -> p.getSecond().getMaterials(modelGetter, missingTextureErrors).stream())
