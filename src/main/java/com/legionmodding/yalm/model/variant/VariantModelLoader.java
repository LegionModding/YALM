...
public class VariantModelLoader implements IModelLoader<VariantModelGeometry>
{
    ...
    {
        final IUnbakedModel base = deserializationContext.deserialize(JSONUtils.getAsJsonObject(modelContents, KEY_BASE), BlockModel.class);

        final Evaluator evaluator = new Evaluator();

        if (modelContents.has(KEY_EXPANSIONS))
        {
            JsonArray expansions = JSONUtils.getAsJsonArray(modelContents, KEY_EXPANSIONS);
        ...
        {
            JsonObject partData = JSONUtils.getAsJsonObject(    modelContents, KEY_VARIANTS);
